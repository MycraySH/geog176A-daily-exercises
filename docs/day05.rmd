...
filter(date == max(date)) %>%
group_by(state) %>% 
summarize(current_cases = sum(cases)) %>% 
arrange(-current_cases) %>% 
ungroup()  %>%
head(5)
```
...
filter(date == max(date)) %>% 
group_by(county) %>% 
mutate(deathratiocase = (deaths/cases)*100) %>% 
ungroup()  %>%
select(county,deathratiocase) %>% 
arrange(-deathratiocase) %>% 
head(5)
```
...
filter(date == max(date)) %>% 
group_by(state) %>% 
summarize(sumdeath = sum(deaths),sumcases = sum(cases)) %>% 
mutate(deathratiocase = (sumdeath/sumcases)*100) %>% 
ungroup()  %>%
select(state,deathratiocase) %>% 
filter(deathratiocase<=100) %>% 
arrange(-deathratiocase) %>% 
head(5)
```
